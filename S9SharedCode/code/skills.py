...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer":
        # Desktop twin of the Browser branch: the Computer-Use skill owns
        # its own cascade (hotkey → a11y → cdp → vision) over cua-driver and
        # never touches the LLM tool/text channel, so we bypass render_prompt
        # and the gateway-chat dispatch and hand off to ComputerSkill.run.
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
            skill="computer",
            inputs=node_dict.get("inputs") or [],

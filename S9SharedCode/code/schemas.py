...
class ComputerOutput(BaseModel):
    """Session 9: typed payload the Computer-Use skill writes into
    AgentResult.output. Desktop twin of BrowserOutput.

    `path` is the cascade tier the skill actually STOPPED on (the layer
    that produced the answer): extract (read-only AX/clipboard), hotkey
    (deterministic key sequence), a11y (AX tree + judgment), cdp (Electron
    page tool), or vision (set-of-marks). `cascade` lists every tier the
    skill attempted, in order, so the escalation path is visible exactly
    like the Browser cascade. Replay and the Planner's failure routing read
    `path` as they do for Browser. `trajectory_dir` is the cua-driver
    recording directory — the assignment's submission evidence.
    """

    app: str
    goal: str
    path: Literal["extract", "hotkey", "a11y", "cdp", "vision"]
    turns: int = 0
    content: str | None = None
    actions: list[dict] = Field(default_factory=list)

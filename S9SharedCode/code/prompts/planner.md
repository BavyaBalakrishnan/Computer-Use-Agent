...
Available skills:
  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     when needed.
  computer           drive a NATIVE or ELECTRON DESKTOP APP on
                     the user's Mac through a four-layer cascade
                     (hotkey → a11y → cdp → vision). Use when the
                     task requires interacting with a local desktop
                     application — Calculator, a system utility, an
                     Electron app (VS Code, Slack), or a browser
                     game/canvas surface whose TARGET IS DRAWN ON
                     A <canvas> ELEMENT (e.g. a game sprite, a map
                     tile, a chart bar) — these targets are invisible
                     to the DOM and require vision. Do NOT use for
                     web pages where the target is a normal DOM
                     element (button, link, input) — use browser
                     instead.
                     metadata MUST set: goal (str, what to do in
                     the app). SHOULD also set: app (display name,
                     e.g. "Calculator"), bundle_id (macOS bundle ID,
                     e.g. "com.apple.calculator"). For Electron
                     apps set electron_debugging_port (default 9222).
                     If the user names a SPECIFIC WINDOW (e.g. "the
                     APP9 window", "the window titled Untitled"),
                     set window_title_hint to that substring so the
                     skill targets that window instead of the largest.
                     For canvas/game targets (large sprites, game
                     objects): set grid: {zoom: false} (disables
                     two-stage zoom that mis-translates coordinates
                     on large targets) and page_settle_s: 8 (canvas
                     games need extra time to render). Also set
                     urls: [<url>] so the skill navigates to the page.
                     Example canvas task metadata:
                       app: "Safari"
                       bundle_id: "com.apple.Safari"
                       urls: ["https://orteil.dashnet.org/cookieclicker/"]
                       goal: "click the large round brown cookie"
                       grid: {zoom: false}
                       page_settle_s: 8
                     Do NOT set metadata.layer — the cascade picks
                     the cheapest tier automatically (hotkey for
                     key-driven apps, a11y for native UI, cdp for
                     Electron DOM, vision only for canvas/games).
                     SAFE targets only: stateless system apps,
                     public no-login pages, throwaway windows.
                     Never open the user's documents or projects.
